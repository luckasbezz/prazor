...
type ResolutionRow = {
  ...
  replacement_unit_value: number | string;
  credit_amount: number | string;
  recovered_value: number | string;
  notes: string | null;
  created_at: string;
};
...
export default async function ExchangesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const auditPromise = canViewAudit
    ? supabaseRest<AuditRow[]>(`audit_logs?select=id,actor_user_id,action,before_data,after_data,created_at&company_id=eq.${companyId}&entity_type=in.(exchange_requests,exchange_request_resolutions)&order=created_at.desc&limit=24`, auth.accessToken)
    : Promise.resolve([] as AuditRow[]);

  const [products, batches, balances, locations, branches, suppliers, agreements, requests, requestLines, resolutions, settings, unread, auditRows] = await Promise.all([
    ...
    supabaseRest<ResolutionRow[]>(`exchange_request_resolutions?select=exchange_request_id,outcome,accepted_quantity,rejected_quantity,replacement_quantity,replacement_unit_value,credit_amount,recovered_value,notes,created_at&company_id=eq.${companyId}&limit=1500`, auth.accessToken),
  ...
  const resolutionByRequestId = new Map(resolutions.map((resolution) => [resolution.exchange_request_id, resolution]));
  const exchangeRequests: ExchangeRequestItem[] = requests.flatMap((request) => {
    ...
    return [{
      ...
      resolution: resolution ? {
        ...
        acceptedQuantity: Number(resolution.accepted_quantity),
        rejectedQuantity: Number(resolution.rejected_quantity),
        replacementQuantity: Number(resolution.replacement_quantity),
        replacementUnitValue: Number(resolution.replacement_unit_value),
        creditAmount: Number(resolution.credit_amount),
        recoveredValue: Number(resolution.recovered_value),
...
function activityTitle(row: AuditRow) {
  ...
  if ("recovered_value" in data) return "Resultado financeiro registrado";
...
function activityDetail(row: AuditRow) {
  const data = row.after_data ?? row.before_data ?? {};
  if ("recovered_value" in data) {
    return `Recuperado ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(Number(data.recovered_value ?? 0))}`;
  }
